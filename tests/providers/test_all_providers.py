...
def test_provider(
    ...
    Args:
        ...
        receive_currency: Optional currency code for receiving (needed for some providers)
    ...
    try:
        ...
        if receive_currency:
            logger.info(f"Testing {provider_name} for {send_currency} to {receive_country} ({receive_currency})")
        else:
        ...
        # Add receive_currency if specified
        if receive_currency:
            rate_params['receive_currency'] = receive_currency
            
        rate_info = provider.get_exchange_rate(**rate_params)
        ...
        # Add receive_currency if specified
        if receive_currency:
            quote_params['receive_currency'] = receive_currency
            
        quote = provider.get_quote(**quote_params)
        ...
        result.update({
            ...
            'receive_currency': quote.get('receive_currency', receive_currency),  # Use provided currency as fallback
...
def main():
    ...
    parser.add_argument("--receive-currency", type=str, help="Currency code to receive (e.g., INR, MXN)")
    ...
    receive_currency = args.receive_currency.upper() if args.receive_currency else None
    ...
    if receive_currency:
        logger.info(f"Testing {len(providers_to_test)} providers for {amount} {currency} to {country} ({receive_currency})")
    else:
    ...
    for provider_name in providers_to_test:
        try:
            result = test_provider(provider_name, amount, currency, country, receive_currency)
