...
try:
    ...
    from apps.providers.orbitremit.exceptions import OrbitRemitError
except ImportError:
    logger.error("Failed to import OrbitRemit provider. Make sure you're running this from the project root.")
...
def print_result(result):
    ...
    if result["success"]:
        if "fee" in result and result["fee"] is not None:
            logger.info("✅ Fee Info Success: %s %s → Fee: %s", 
                      result["send_amount"], result["send_currency"], 
                      result["fee"])
    else:
...
def test_fee_info():
    ...
    # List of test cases
    test_cases = [
        {"send_currency": "AUD", "payout_currency": "PHP", "send_amount": Decimal("200000")},
        {"send_currency": "NZD", "payout_currency": "INR", "send_amount": Decimal("1000")},
        {"send_currency": "GBP", "payout_currency": "PKR", "send_amount": Decimal("500")},
        {"send_currency": "EUR", "payout_currency": "BDT", "send_amount": Decimal("1500")},
        {"send_currency": "CAD", "payout_currency": "LKR", "send_amount": Decimal("750")},
        {"send_currency": "USD", "payout_currency": "VND", "send_amount": Decimal("2000")},
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        logger.info("\nTest %d/%d: Converting %s %s to %s", 
                  i, len(test_cases),
                  test_case["send_amount"], test_case["send_currency"], 
...
def test_quotes():
    ...
    # List of test cases
    test_cases = [
        {"amount": Decimal("200000"), "source_currency": "AUD", "target_currency": "PHP"},
    ...
    for i, test_case in enumerate(test_cases, 1):
        logger.info("\nTest %d/%d: Converting %s %s to %s", 
                  i, len(test_cases),
                  test_case["amount"], test_case["source_currency"], 
                  test_case["target_currency"])
        
        result = provider.get_quote(**test_case)
        
        if result["success"]:
            logger.info("✅ Quote Success: %s %s", 
                      test_case["amount"], test_case["source_currency"])
...
def test_error_handling():
    ...
    for i, test in enumerate(error_tests, 1):
        ...
        if not result["success"] and result.get("error_message") and test["expected_error"] in result["error_message"]:
            logger.info("✅ Test passed: success=False")
            logger.info("   Error message: %s", result["error_message"])
            success_count += 1
        else:
            logger.error("❌ Test failed: Expected error containing '%s', got: %s", 
                       test["expected_error"], result.get("error_message"))
