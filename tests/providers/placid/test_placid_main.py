        ...
        try:
            ...
            if corridor_val in content:
                ...
                if match:
                    ...
                    logger.warning(f"⚠️ Could not find exchange rate for {currency_code} in response")
...
def test_task_types():
    ...
    for result in results:
        ...
        logger.info(f"{status} {result['task_type']}: Success={result['success']}, Rate={result['rate']}, Content Length={result['content_length']}")
...
def run_api_tests():
    ...
    for result in corridor_results:
        ...
        logger.info(f"{status} {result['corridor']} ({result['currency']}): Success={result['success']}{rate_info}")
        if result["success"]:
            success_count += 1
    ...
    if success_count > 0 and task_type_success:
        logger.info(f"✅ API tests passed for {success_count}/{len(corridor_results)} corridors and task types")
        return True
    else:
...
def test_factory():
    ...
    try:
        from apps.providers import get_provider_by_name, list_providers
        from apps.providers.placid import PlacidProvider
        ...
        for corridor in test_corridors:
            ...
            if result.get("success"):
                ...
                logger.warning(f"⚠️ Failed to get rate for {corridor}: {result.get('error_message')}")
        
        if success_count > 0:
            logger.info(f"✅ Factory tests passed for {success_count}/{len(test_corridors)} corridors")
            return True
        else:
        ...
        logger.info("Make sure the provider is registered in the factory and all dependencies are installed.")
        return False
...
def print_result(result):
    ...
    if result.get("success"):
        logger.info(f"✅ Quote Success: {result['send_amount']} {result.get('source_currency', 'source')} → "
    ...
    try:
        ...
        for i, test in enumerate(combined_tests):
            ...
            if rate_result.get("success"):
                ...
                logger.warning(f"⚠️ Direct exchange rate failed: {rate_result.get('error_message')}")
            ...
            if quote_result.get("success"):
                ...
                if abs(calculated_receive - actual_receive) < 0.01:  # Allow small rounding differences
                    logger.info("✅ Receive amount calculation is correct")
                else:
                    logger.warning(f"⚠️ Calculation mismatch: {calculated_receive} != {actual_receive}")
...
def test_error_handling():
    ...
    try:
        from apps.providers import get_provider_by_name
        from decimal import Decimal
        from apps.providers.placid.exceptions import PlacidError
        ...
        for i, test in enumerate(error_tests):
            ...
            try:
                ...
                if result.get("success") == test["expect_success"]:
                    ...
                    logger.error(f"❌ Test failed: Expected success={test['expect_success']}, got {result.get('success')}")
