...
async def test_get_quote_send_amount(provider, mock_response):
    ...
    with patch("requests.Session.post", return_value=mock_response):
        result = provider.get_quote(
            send_amount=Decimal("1000.00"), send_currency="SGD", receive_currency="INR"
        )
...
def test_api_error_handling(provider):
    ...
    mock_error_response.json.return_value = {"errors": ["Invalid request parameters"]}
...
def test_auth_error_handling(provider):
    ...
    mock_auth_error.json.return_value = {"errors": ["Authentication failed"]}
...
@pytest.mark.parametrize(
    "payment_method",
    [{"swift": True}, {"cash_pickup": True}, {"wallet": True}, {"business": True}],
)
