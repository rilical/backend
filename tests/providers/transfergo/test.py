        ...
        try:
            ...
            if "preferred_receive_currency" in test_case:
                if result["destination_currency"] == test_case["preferred_receive_currency"]:
                    print(f"✅ Preferred currency ({test_case['preferred_receive_currency']}) was used successfully")
                else:
                    print(f"❌ Preferred currency ({test_case['preferred_receive_currency']}) was not used. Using {result['destination_currency']} instead")
...
def test_supported_countries_and_currencies():
    ...
    total_currencies = len(set([curr for curr_list in countries_currencies.values() for curr in curr_list]))
    multi_currency_countries = [country for country, currencies in countries_currencies.items() if len(currencies) > 1]
    ...
    columns = 5
    send_country_chunks = [send_countries[i:i+columns] for i in range(0, len(send_countries), columns)]
