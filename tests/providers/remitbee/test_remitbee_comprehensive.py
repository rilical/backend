...
try:
    ...
    print(
        "Cannot import RemitbeeProvider. Make sure you're running this script from the project root directory."
    )
    sys.exit(1)
...
def run_comprehensive_tests(html_file=None):
    ...
    for i, country_data in enumerate(countries):
        ...
        print(f"\n{'='*80}")
        print(
            f"Testing corridor {i+1}/{len(countries)}: CAD → {country_code} ({currency_code}) - {country_name}"
        )
            ...
            try:
                ...
                if result:
                    ...
                    if not result.get("supported", False) or "error" in result:
                        print(
                            f"  ❌ UNSUPPORTED - {result.get('error', 'No specific error message')}"
                        )
                        ...
                        print(
                            f"    Receive Amount: {result.get('receive_amount')} {result.get('receive_currency')}"
                        )
