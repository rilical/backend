...
def test_tariffs():
    ...
    for case in test_cases:
        ...
        logger.info(f"Corridor: {case['sending_country']} ({case['sending_currency']}) -> {case['receiving_country']} ({case['receiving_currency']})")
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.info(f"Send Amount: {format_currency(result['sending_amount'], case['sending_currency'])}")
                logger.info(f"Receive Amount: {format_currency(result['receiving_amount'], case['receiving_currency'])}")
                logger.info(f"Total Cost: {format_currency(result['total_cost'], case['sending_currency'])}")
            else:
                logger.error("✗ FAILED")
    ...
    for case in test_cases:
        ...
        try:
            ...
            if result["success"]:
                logger.info("✓ SUCCESS")
                logger.info("-" * 30)
                logger.info(f"Send Amount: {format_currency(result['send_amount'], result['send_currency'])}")
                logger.info(f"Receive Amount: {format_currency(result['receive_amount'], result['receive_currency'])}")
                logger.info(f"Exchange Rate: {result['rate']:.4f}")
                logger.info(f"Fee: {format_currency(result['fee'], result['send_currency'])}")
                logger.info(f"Total Cost: {format_currency(result['total_cost'], result['send_currency'])}")
            else:
                logger.error("✗ FAILED")
                logger.error(f"Error: {result.get('error', 'Unknown error')}")
                log_api_response(result)
...
def test_exchange_rates():
    ...
    for case in test_cases:
        ...
        try:
            result = provider.get_exchange_rate(**{k: v for k, v in case.items() if k != 'description'})
            if result["success"]:
                logger.info("✓ SUCCESS")
                logger.info("-" * 30)
