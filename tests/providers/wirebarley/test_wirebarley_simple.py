...
def get_wirebarley_quotes():
    ...
    session.headers.update({"Referer": homepage_url})
...
def test_calculate_quotes():
    ...
    try:
        ...
        for source, _ in [("USD", ""), ("NZD", ""), ("GBP", "")]:
            ...
            if corridors_result["success"]:
                ...
                if corridors_result["corridors"]:
                    ...
                    print(
                        f"First corridor: {first['source_currency']} to {first['target_currency']}"
                    )
        ...
        for amount, source, dest in test_cases:
            ...
            quote_result = provider.get_quote(
                amount=amount, source_currency=source, destination_currency=dest
            )
    ...
    for amount, source, dest in test_cases:
            ...
            for fee_struct in dest_rate["transferFees"]:
                ...
                if min_amt <= amount <= max_amt:
                    ...
                    if "threshold1" in fee_struct and fee_struct["threshold1"] is not None:
                        ...
                        if (
                            amount >= threshold1
                            and "fee2" in fee_struct
                            and fee_struct["fee2"] is not None
                        ):
                    ...
                    if "threshold2" in fee_struct and fee_struct["threshold2"] is not None:
                        ...
                        if (
                            amount >= threshold2
                            and "fee3" in fee_struct
                            and fee_struct["fee3"] is not None
                        ):
        ...
        print(
            f"  QUOTE: {amount} {source} = {destination_amount:.2f} {dest} (Rate: {exchange_rate}, Fee: {fee})"
        )
