...
class TestWireBarleyLive(unittest.TestCase):
    ...
    def test_rate_calculation(self):
        ...
        if "wbRateData" in php_rate_data:
            ...
            print(
                f"  Thresholds: {[wb_rate_data.get(f'threshold{i if i else ''}', None) for i in range(9) if wb_rate_data.get(f'threshold{i if i else ''}') is not None]}"
            )
        ...
        try:
            ...
            if corridors["success"]:
                ...
                for i, corridor in enumerate(corridors["corridors"][:3]):
                    print(
                        f"  {i+1}. {corridor['source_currency']} to {corridor['target_currency']}"
                    )
