...
def test_xoom_aggregator(amount: Decimal, source_currency: str, target_country: str):
    ...
    """
    logger.info(f"Testing Xoom Aggregator Provider: {source_currency} -> {target_country} for {amount}")
...
def test_all_corridors():
    ...
    with XoomAggregatorProvider() as provider:
        for corridor in TEST_CORRIDORS:
            logger.info(f"Testing corridor: {corridor['source_currency']} -> {corridor['target_country']}")
...
def main():
    ...
    if args.test_all:
        ...
        if args.output:
            ...
            for result in results:
                status = "✅" if not result.get("success") else "❌"  # For error tests, we expect failure
                format_status = "✅" if result.get("valid_format") else "❌"
