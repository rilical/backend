...
from providers.singx.singx_mappings import (
    API_CONFIG,
    COUNTRY_CODES,
    DEFAULT_VALUES,
    SUPPORTED_CORRIDORS,
        ...
        **kwargs,
        ...
        try:
            ...
            if not is_country_supported(destination_country):
                raise SingXValidationError(
                    f"Unsupported destination country: {destination_country}"
                )
            ...
            if not all(
                k in data for k in ("sendAmount", "receiveAmount", "singxFee", "exchangeRate")
            ):
