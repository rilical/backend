    ...
    with RIAProvider() as provider:
        for corridor in TEST_CORRIDORS:
            ...
            if corridor["expected_success"] is not None:
                assert (
                    result["success"] == corridor["expected_success"]
                ...
                if "available_delivery_methods" in result:
                    ...
                    for method in result["available_delivery_methods"]:
                        logger.info(
                            ...
                            + (
                                f"Rate: {method.get('exchange_rate', 'N/A')}"
                                if "exchange_rate" in method
                                else "No rate"
                            )
                        )
            ...
            if result["success"] and "available_delivery_methods" in result:
                ...
                if methods:
                    ...
                    for idx, method in enumerate(methods, 1):
                        ...
                        logger.info(
                            f"  {idx}. {method['method_name']} ({method['method_code']}) → {method['standardized_name']} | {rate_info} {best_rate}"
                        )
                    ...
                    if len(methods) > 1:
                        ...
                        if alt_result["success"]:
                            logger.info(
                                f"  ✅ Quote with {alt_method}: Rate {alt_result['exchange_rate']}, Amount {alt_result['destination_amount']}"
                            )
                        else:
                            logger.info(
                                f"  ❌ Failed to get quote with {alt_method}: {alt_result['error_message']}"
                            )
            else:
                logger.warning(
                    f"No delivery methods found for {corridor['name']} or request failed"
                )

                # Try with a different payment method to see if that reveals delivery methods
                logger.info(
                    f"Trying with different payment method (BankAccount) for {corridor['name']}..."
                )
                alt_result = provider.get_quote(
                    amount=Decimal(str(corridor["send_amount"])),
                    source_currency=corridor["send_currency"],
                ...
                if alt_result["success"] and "available_delivery_methods" in alt_result:
                    ...
                    logger.info(
                        f"Found {len(methods)} delivery methods for {corridor['name']} with BankAccount payment"
                    )
                    for idx, method in enumerate(methods, 1):
                        logger.info(
...
def test_error_handling() -> None:
    ...
    with RIAProvider() as provider:
        ...
        logger.info(
            f"Invalid source country: success={result['success']}, error={result['error_message']}"
        )
        ...
        logger.info(
            f"Negative amount: success={result['success']}, error={result['error_message']}"
        )
