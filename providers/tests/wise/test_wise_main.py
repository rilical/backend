...
from providers.wise.exceptions import (
    ...
    def test_common_corridors(self):
        ...
        for corridor in corridors:
            ...
            self.test_logger.info(
                f"Testing corridor: {send_country}({send_currency})->{receive_country} with amount {amount}"
            )
            ...
            try:
                ...
                self.test_logger.error(
                    f"Error testing corridor {send_country}({send_currency})->{receive_country}: {str(e)}"
                )
                self.test_logger.error(traceback.format_exc())
                results.append(
                    {
    ...
    def test_quotes_target_amount(self):
        ...
        try:
            ...
            quote_data = self.provider._create_unauthenticated_quote(
                source_currency="EUR", target_currency="USD", target_amount=100.0
            )
            ...
            # Log the results
            self.test_logger.info(
                f"Source amount: {quote_data.get('sourceAmount', 'Not provided')}"
            )
    ...
    def test_discover_supported_methods(self):
        ...
        for corridor in corridors:
            ...
            try:
                ...
                self.test_logger.error(
                    f"Failed to test corridor {send_country}->{receive_country}: {str(e)}"
                )
                self.test_logger.error(traceback.format_exc())
                results.append(
                    {
