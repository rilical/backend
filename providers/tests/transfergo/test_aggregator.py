...
# Import the provider
from providers.transfergo.aggregator_integration import TransferGoProvider
    ...
    for dest_country in CASH_PICKUP_COUNTRIES:
        try:
            ...
            if dest_country in COUNTRY_CURRENCIES:
                ...
                logger.info(
                    f"Testing cash pickup: {source_currency} ({source_country}) → "
    ...
    for dest_country in MOBILE_WALLET_COUNTRIES:
        try:
            ...
            if dest_country in COUNTRY_CURRENCIES:
                ...
                logger.info(
                    f"Testing mobile wallet: {source_currency} ({source_country}) → "
                    f"{dest_currency} ({dest_country})"
                )
            ...
            if dest_countries:
                ...
                if dst_country in COUNTRY_CURRENCIES:
                    ...
                    if (
                        ...
                        dst_country,
                        dst_currency,
                    ) in POPULAR_CORRIDORS:
                        continue

                    logger.info(
                        f"Testing random corridor: {src_currency} ({src_country}) → "
...
def main():
    ...
    parser.add_argument(
        "--test-cash-pickup",
        action="store_true",
        help="Test cash pickup delivery method",
    )
    parser.add_argument(
        "--test-mobile-wallet",
        action="store_true",
    ...
    try:
        if args.comprehensive:
            ...
            test_with_receive_amount(provider, "5000", "GB", "GBP", "PH", "PHP")
