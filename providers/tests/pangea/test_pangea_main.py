...
class TestPangeaProviderRealAPI(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        root_log_file = os.path.join(
            cls.logs_dir, f"pangea_tests_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def tearDown(self):
        ...
        test_log_file = os.path.join(
            self.logs_dir,
            f"{self._testMethodName}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
        )
    ...
    def save_response_data(self, data, prefix, error_reason=None):
        ...
        response_data = {"timestamp": datetime.now().isoformat(), "data": data}
        if error_reason:
    ...
    def test_get_exchange_rate_real(self):
        ...
        """
        test_method_log = os.path.join(
            self.logs_dir,
            f"test_get_exchange_rate_real_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
        )
        ...
        try:
            ...
            result = self.provider.get_exchange_rate(
                send_amount=Decimal("100.00"), send_currency="USD", receive_country="MX"
            )
    ...
    def test_common_corridors(self):
        ...
        test_method_log = os.path.join(
            self.logs_dir,
            f"test_common_corridors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
        )
            ...
            for case in test_cases:
                ...
                try:
                    ...
                    if result:
                        ...
                        test_logger.info(
                            f"Success! Rate: {result['exchange_rate']}, Fee: {result['transfer_fee']}"
                        )
                        test_logger.info(
                            f"Recipient gets: {result['receive_amount']} {result['receive_currency']}"
                        )
                        test_logger.info(f"Response data saved to: {saved_file}")
                    else:
    ...
    def test_discover_supported_methods(self):
        ...
        test_method_log = os.path.join(
            self.logs_dir,
            f"test_discover_supported_methods_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
        )
        ...
        try:
            ...
            test_logger.info(
                f"Discovering supported payment methods for {len(corridors)} corridors"
            )
            ...
            for (
                ...
                try:
                    ...
                    test_logger.info(
                        f"Found {len(exchange_rates)} exchange rate options for {corridor_name}"
                    )
                    ...
                    test_logger.info(
                        f"Found {len(payment_methods)} unique payment methods: {', '.join(payment_methods)}"
                    )
