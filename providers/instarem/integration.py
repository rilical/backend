    ...
    def get_quote(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.info(
                f"Requesting payment methods from InstaRem with params: {payment_params}"
            )
            ...
            if not payment_data.get("success", False) or not payment_data.get("data"):
                error_msg = payment_data.get(
                    "message", "Failed to get payment methods from InstaRem"
                )
                raw_result["error_message"] = error_msg
                return self.standardize_response(
                    raw_result, provider_specific_data=kwargs.get("include_raw", False)
                )
            ...
            response = self.session.get(
                f"{self.BASE_URL}{self.QUOTE_ENDPOINT}", params=quote_params, timeout=30
            )
