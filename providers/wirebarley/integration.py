...
from providers.utils.country_currency_standards import validate_corridor
from providers.wirebarley.exceptions import (
    WireBarleyAPIError,
    WireBarleyAuthError,
...
class WireBarleyProvider(RemittanceProvider):
    ...
    def _get_selenium_cookies(self):
        ...
        if not email or not password:
            self.logger.error(
                "WIREBARLEY_EMAIL and WIREBARLEY_PASSWORD must be set for Selenium automation"
            )
            return None
    ...
    def _ensure_valid_session(self):
        ...
        now = time.time()
        if (self.session_timestamp is None) or (
            (now - self.session_timestamp) > self.SESSION_VALID_DURATION
        ):
            self.logger.info("WireBarley session expired or missing, re-initializing")
            self._initialize_session()

    def _fetch_api_data(
        self, send_currency: str, receive_country_code: str, amount: Decimal
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get("status") != "success":
                ...
                return {"success": False, "error_message": f"API error: {error_msg}"}
            ...
            return {"success": False, "error_message": f"Request error: {str(e)}"}
        except Exception as e:
            return {"success": False, "error_message": f"Unexpected error: {str(e)}"}

    def _pick_threshold_rate(
        self, corridor_obj: Dict[str, Any], amount: Decimal
    ) -> Optional[float]:
    ...
    def _find_fee_in_array(
        self, fees_array: List[Dict[str, Any]], amount: Decimal
    ) -> Optional[float]:
    ...
    def get_exchange_rate(
        ...
        try:
            ...
            if not country_code:
                self.logger.info(
                    f"Unable to determine country code for currency {receive_currency}"
                )
            ...
            public_url = (
                f"{self.BASE_URL}/my/remittance/api/v1/exrate/{source_country}/{send_currency}"
            )
                ...
                if data.get("data") is not None and "exRates" in data["data"]:
                    ...
                    for rate_data in data["data"]["exRates"]:
                        if rate_data.get("currency") == receive_currency:
                            ...
                            self.logger.info(
                                f"Rate found for {send_currency} to {receive_currency}: {wb_rate}"
                            )
                    ...
                    return self._try_authenticated_api(
                        send_amount, send_currency, receive_currency, country_code
                    )
            else:
        ...
        **kwargs,
        ...
        try:
            ...
            if destination_currency == "MXN":
                self.logger.info(
                    f"WireBarley does not support {source_currency} to {destination_currency} corridor"
                )
                return self.standardize_response(
                    {
                        "success": False,
            ...
            if not dest_country:
                self.logger.info(
                    f"Unable to determine country code for currency {destination_currency}"
                )
                return self.standardize_response(
                    {
                        "success": False,
            ...
            if not amount:
                return self.standardize_response(
                    {"success": False, "error_message": "Amount is required"}
                )
    ...
    def get_corridors(self, source_currency: str = "USD") -> Dict[str, Any]:
        ...
        if not self.session:
            failure_response[
                "error_message"
            ] = "No valid session. Check WireBarley cookies or Selenium login."
            return failure_response
        ...
        try:
            ...
            # Use public endpoint to get rates for all corridors in a single request
            public_url = (
                f"{self.BASE_URL}/my/remittance/api/v1/exrate/{source_country}/{source_currency}"
            )
            ...
            if (
                cache_key in self._corridors_cache
                and cache_key in self._cache_timestamp
                and (time.time() - self._cache_timestamp[cache_key]) < self._cache_duration
            ):
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Public API returned status {response.status_code} for corridors"
                )
                # Fall back to old method
                return self._try_authenticated_corridors_api(source_currency)
    ...
    def _try_authenticated_corridors_api(self, source_currency):
        ...
        try:
            ...
            # Create response
            result = {"success": True, "provider_id": self.name, "corridors": corridors}
