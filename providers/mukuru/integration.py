...
from providers.mukuru.exceptions import (
    MukuruApiError,
...
from providers.mukuru.mapping import (
    COUNTRY_TO_CURRENCY,
    CURRENCY_ID_MAPPING,
    DELIVERY_METHODS,
    PAYMENT_METHODS,
...
class MukuruProvider(RemittanceProvider):
    ...
    def standardize_response(
        ...
        final_target_currency = local_data.get("target_currency") or local_data.get(
            "receive_currency", ""
        )
    ...
    def get_currency_id(
        self, from_country: str, to_country: str, delivery_method: str = None
    ) -> int:
        """
        ...
        if corridor_id is None:
            if to_country == "ZW":
                logger.warning(
                    f"No currency ID for {from_country}->{to_country}, using default USD Cash (18)"
                )
                corridor_id = 18
            else:
                logger.warning(
                    f"No currency ID for {from_country}->{to_country}, using default (1)"
                )
                corridor_id = 1
    ...
    def get_quote(
        self, amount: Decimal, source_currency: str, target_country: str, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        from_country_code = kwargs.pop("from_country", None) or kwargs.pop(
