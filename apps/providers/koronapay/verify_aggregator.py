...
def print_response(title, response):
    ...
    if "destination_amount" in response and "destination_currency" in response:
        print(f"Receive: {format_currency(response['destination_amount'], response['destination_currency'])}")
    ...
    if "exchange_rate" in response:
        ...
        if "rate" in response and response["rate"] != response["exchange_rate"]:
            print(f"⚠️  Warning: 'rate' ({response['rate']}) differs from 'exchange_rate' ({response['exchange_rate']})")
        else:
    ...
    if "source_currency" in response and "send_currency" in response.get("raw_data", {}):
        ...
        if source.upper() != send.upper():
            print(f"⚠️  Warning: 'source_currency' ({source}) differs from 'send_currency' ({send})")
    ...
    if "destination_currency" in response and "target_currency" in response:
        ...
        if dest.upper() != target.upper():
            print(f"⚠️  Warning: 'destination_currency' ({dest}) differs from 'target_currency' ({target})")
        else:
