...
TEST_CORRIDORS = {
    ...
    "UNSUPPORTED": [
        {"name": "JPY → South Africa (ZAR)", "source": "JPY", "country": "ZA", "currency": "ZAR"},
        {"name": "EUR → Fiji (FJD)", "source": "EUR", "country": "FJ", "currency": "FJD"},
    ]
}
    ...
    with RemitlyProvider() as provider:
        ...
        for corridor in TEST_CORRIDORS["UNSUPPORTED"]:
            ...
            try:
                ...
                logger.info(f"Result: {'SUCCESS' if result['success'] else 'FAILED'} - {result.get('error_message', 'No error message')}")
            except Exception as e:
                logger.error(f"ERROR testing {corridor['name']}: {str(e)}")
...
def test_support_methods():
    ...
    with RemitlyProvider() as provider:
        ...
        for country in sample_countries:
            ...
            logger.info(f"Currency for {provider._convert_country_code(country)}: {provider._get_currency_for_country(provider._convert_country_code(country))}")
        ...
        delivery_methods = ["BANK_DEPOSIT", "CASH_PICKUP", "HOME_DELIVERY", "MOBILE_WALLET"]
