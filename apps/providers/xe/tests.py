...
def test_quotes_api(from_currency, to_country, amount):
    ...
    logger.info(f"Testing quotes API: {from_currency} ({from_country}) -> {to_currency} ({to_country}), amount={amount}")
...
def test_website_endpoints():
    ...
    endpoints = [
        ...
        ("Money Transfer Page", f"{MONEY_TRANSFER_URL}/?Amount=1000&FromCurrency=USD&ToCurrency=INR")
    ]
...
def main():
    ...
    parser.add_argument("--from", dest="from_currency", type=str, default="USD", help="Source currency (e.g., USD, GBP)")
    parser.add_argument("--to", dest="to_country", type=str, default="IN", help="Target country code (e.g., IN, PH)")
    parser.add_argument("--amount", type=float, default=1000, help="Amount to send")
    parser.add_argument("--all", action="store_true", help="Run all tests including website endpoint tests")
