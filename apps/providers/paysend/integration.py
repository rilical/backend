...
class PaysendProvider(RemittanceProvider):
        ...
        while attempt < max_retries:
            try:
                ...
                if method == "GET":
                    ...
                    response = self.session.post(url, data=request_data, headers=request_headers, timeout=30)
                else:
    ...
    def get_quote(self, from_country, to_country, from_currency, to_currency, amount,
        ...
        if not self.from_country or not self.to_country:
            ...
            return {"success": False, "error_message": "Source and destination countries are required"}
        ...
        if not self.send_currency or not self.receive_currency:
            ...
            return {"success": False, "error_message": "Source and destination currencies are required"}
    ...
    def get_exchange_rate(
        ...
        """
        logger.info(f"Getting exchange rate from {send_currency} ({from_country}) to {receive_currency} ({receive_country})")
        ...
        try:
            ...
            if quote_result["success"]:
                ...
                logger.info(f"Successfully retrieved Paysend exchange rate: {response['exchange_rate']}")
            else:
        ...
        try:
            response = self.session.get(
                'https://paysend.com/api/init-session',
                headers=headers,
                timeout=30
            )
            
            if response.status_code != 200:
                logger.error(f"Failed to initialize Paysend session. Status: {response.status_code}")
                self._session_token = None
                return False
    ...
    def _extract_exchange_data(self, response_data):
        ...
        if 'country' in response_data and hasattr(self, 'send_currency') and hasattr(self, 'receive_currency'):
            ...
            if from_currency_id and to_currency_id and hasattr(self, 'amount') and float(self.amount) > 0:
        ...
        try:
            ...
            # Check for rate info in an alternative format (catalog data)
                ...
                if from_country and to_country:
                    response["success"] = True
                    response["send_currency"] = self.send_currency if hasattr(self, 'send_currency') else None
                    response["receive_currency"] = self.receive_currency if hasattr(self, 'receive_currency') else None
                    response["send_amount"] = float(self.amount) if hasattr(self, 'amount') and self.amount else None
