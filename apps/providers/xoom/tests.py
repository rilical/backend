...
from apps.providers.xoom.exceptions import XoomError, XoomValidationError, XoomAuthenticationError
    ...
    def test_fee_table_api_live(self):
        ...
        for country in test_corridors:
            for amount in test_amounts:
                ...
                logger.info(f"Testing exchange rate for {amount} USD to {country['name']} ({currency_code})")
                ...
                try:
                    ...
                    logger.info(f"  Receive: {result['receive_amount']} {result['receive_currency']}")
...
class TestXoomIntegration:
    ...
    def test_get_exchange_rates_multiple_corridors(self):
        ...
        test_corridors = [
            ...
            {"source": "USD", "destination": "PH", "currency": "PHP", "name": "Philippines"},
            {"source": "USD", "destination": "IN", "currency": "INR", "name": "India"},
            {"source": "USD", "destination": "CO", "currency": "COP", "name": "Colombia"},
        ...
        for corridor in test_corridors:
            for amount in test_amounts:
                logger.info(f"Testing {amount} {corridor['source']} to {corridor['destination']} ({corridor['currency']})")
                ...
                try:
                    ...
                    # Log success details
                    logger.info(f"SUCCESS: Exchange rate {result['exchange_rate']}, Fee: {result['fee']}")
