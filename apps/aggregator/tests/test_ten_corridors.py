    ...
    if result.get('quotes'):
        ...
        for i, quote in enumerate(result.get('quotes', []), 1):
            table_data.append([
                ...
                f"{quote.get('delivery_time_minutes', 'N/A')} min" if quote.get('delivery_time_minutes') else 'N/A'
            ])
        
        print(tabulate(table_data, headers=[
            '#', 'Provider', 'Rate', 'Fee', 'Recipient Gets', 'Delivery Time'
...
def run_corridor_tests():
    ...
    print(tabulate(summary_table, headers=[
        'Corridor', 'Status', 'Providers', 'Success Rate', 'Time'
    ], tablefmt='pipe'))
    ...
    print("\nOverall Statistics:")
    print(f"Corridors: {successful_corridors}/{total_tests} successful ({successful_corridors/total_tests*100:.1f}%)")
    print(f"Provider Calls: {successful_provider_calls}/{total_provider_calls} successful ({successful_provider_calls/total_provider_calls*100:.1f}%)")
    ...
    print(tabulate(provider_table, headers=[
        'Provider', 'Corridors', 'Success Rate'
    ], tablefmt='pipe'))
