...
def test_provider(provider_class, provider_args=None, **corridor_params):
    ...
    try:
        ...
        if result.get("success"):
            ...
            print(f"Recipient Gets: {result.get('destination_amount', 'N/A')} {corridor_params['dest_currency']}")
            print(f"Delivery Time: {result.get('delivery_time_minutes', 'N/A')} minutes\n")
        else:
            logger.warning(f"FAILED: {provider_name} returned error: {result.get('error_message', 'Unknown error')}")
...
def main():
    ...
    working_providers = [result.get("provider_id") for result in all_results if result.get("success")]
