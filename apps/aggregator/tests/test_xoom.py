...
def run_xoom_test():
    ...
    logger.info(f"Testing XoomProvider with {source_country} → {dest_country} ({source_currency} → {dest_currency}) for {amount}")
    ...
    result = Aggregator.get_all_quotes(
        ...
        amount=amount,
        sort_by="best_rate",
        exclude_providers=[p.__class__.__name__ for p in Aggregator.PROVIDERS if not isinstance(p, XoomProvider)]
    )
