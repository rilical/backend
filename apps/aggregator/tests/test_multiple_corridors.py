...
def run_corridor_test(corridor: Tuple[str, str, str, str, Decimal], sort_by: str = "best_rate") -> Dict[str, Any]:
    """
    ...
    logger.info(f"Testing corridor: {source_country} → {dest_country} ({source_currency} → {dest_currency}) for {amount}, sort by: {sort_by}")
...
def run_comprehensive_test():
    ...
    for corridor in TEST_CORRIDORS:
        ...
        for sort_by in SORTING_METHODS:
            try:
                ...
                logger.exception(f"Error testing corridor {corridor_key} with sort_by={sort_by}: {e}")
                failed_tests += 1
    ...
    corridor_headers = ["Corridor", "Success Rate", "Avg Providers", "Avg Execution Time"]
    corridor_rows = []
    
    for corridor_key, result in corridor_results.items():
        success_rate = (result["success"] / result["total"]) * 100
        avg_providers = sum(test["providers_succeeded"] for test in result["tests"]) / len(result["tests"]) if result["tests"] else 0
        avg_execution_time = sum(test["execution_time"] for test in result["tests"]) / len(result["tests"]) if result["tests"] else 0
