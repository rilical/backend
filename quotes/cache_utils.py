...
from .key_generators import (
    get_corridor_cache_key,
    get_corridor_rate_cache_key,
    get_provider_cache_key,
    get_quote_cache_key,
)
...
def cache_corridor_rate_data(
    source_country, dest_country, source_currency, dest_currency, provider_data
):
    """
...
def get_quotes_from_corridor_rates(
    ...
    logger.info(
        f"Using cached corridor rate data for {len(rate_data.get('providers', []))} providers"
    )
    ...
    for provider in rate_data.get("providers", []):
        try:
            ...
            logger.warning(
                f"Error calculating amounts for provider {provider.get('provider_id')}: {str(e)}"
            )
...
def invalidate_corridor_caches(source_country=None, dest_country=None):
    ...
    if source_country and dest_country:
        ...
        corridors = (
            FeeQuote.objects.filter(destination_country=dest_country)
            .values_list("source_country", "destination_country")
            .distinct()
        )
...
def preload_corridor_caches():
    ...
    corridors = FeeQuote.objects.values_list("source_country", "destination_country").distinct()
