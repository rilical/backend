...
def test_corridor(corridor: Tuple[str, str, str, str, str]) -> Dict[str, Any]:
    ...
    logger.info(
        f"Testing corridor: {test_name} ({source_country}->{dest_country}, {source_currency}->{dest_currency})"
    )
    ...
    test_results = {
        ...
        "success_rate": round((success_count / len(all_providers)) * 100, 2)
        if all_providers
        else 0,
    ...
    logger.info(
        f"Corridor {test_name} completed in {elapsed_time:.2f}s. Success rate: {test_results['success_rate']}% ({success_count}/{len(all_providers)})"
    )
    return test_results
...
def print_provider_summary(results: List[Dict[str, Any]]) -> None:
    ...
    sorted_providers = sorted(
        ...
        key=lambda x: (
            x[1]["successful_corridors"],
            x[1]["successful_corridors"] / x[1]["total_corridors"],
        ),
        reverse=True,
    )
...
def create_provider_matrix(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {"providers": sorted_providers, "corridors": corridors, "matrix": matrix}
