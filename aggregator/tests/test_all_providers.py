...
from aggregator.configurator import AggregatorConfig
...
def print_provider_results(results, corridor_name):
    ...
    stats = (
        f"Amount: {amount:.2f} {source_currency}\n"
...
def test_all_providers(corridor_details, sort_by="best_rate", timeout=15, max_workers=5):
    ...
    try:
        ...
        config.set_default_max_workers(
            max_workers
        )  # Limit workers per corridor to avoid overwhelming the system
...
def run_all_tests():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(corridors)) as executor:
        ...
        future_to_corridor = {
            executor.submit(test_all_providers, corridor): corridor for corridor in corridors
        }
...
def create_provider_support_matrix(corridors, corridor_summaries):
    ...
    print(
        "To see the detailed provider support matrix, please run the dedicated corridor analysis tool:"
    )
